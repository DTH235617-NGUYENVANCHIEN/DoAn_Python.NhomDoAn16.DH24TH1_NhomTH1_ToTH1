...
import pyodbc
import utils  # File dùng chung của bạn
from datetime import datetime  # Đây là LỚP datetime
import datetime as dt        # Đây là MÔ-ĐUN datetime (dùng dt.date.today())

# ================================================================
# PHẦN 1: HÀM HELPER TẢI DỮ LIỆU TÀI XẾ
# ================================================================

def _load_driver_data():
    """Hàm helper để tải danh sách tài xế và tạo map tra cứu."""
    driver_id_to_name = {} 
    driver_name_to_id = {} 
    driver_display_list = [""] 
    ...
    if conn is None:
        messagebox.showerror("Lỗi Tải Tài Xế", "Không thể kết nối CSDL để tải danh sách tài xế.")
        return driver_id_to_name, driver_name_to_id, driver_display_list
    ...
    try:
        ...
        sql = """
            ...
            FROM
                NhanVien AS NV
            INNER JOIN
                TaiXe AS TX ON NV.MaNhanVien = TX.MaNhanVien
            WHERE
                NV.TrangThai = 1
            ORDER BY
                NV.HoVaTen
        ...
        for row in rows:
            ...
            if ma_nv and ten_nv:
                driver_id_to_name[ma_nv] = ten_nv
                driver_name_to_id[ten_nv] = ma_nv
                driver_display_list.append(ten_nv)
                
    except Exception as e:
        messagebox.showerror("Lỗi SQL (Tài Xế)", f"Không thể tải danh sách tài xế:\n{e}")
    finally:
        if conn:
            conn.close()
...
# ================================================================
# PHẦN 2: CÁC HÀM CRUD VÀ QUẢN LÝ FORM
# ================================================================
...
def set_form_state(is_enabled, widgets):
    ...
    cbb_state = "readonly" if is_enabled else "disabled" 

    widgets["entry_loaixe"].config(state=entry_state)
    widgets["entry_hangsx"].config(state=entry_state)
    widgets["entry_dongxe"].config(state=entry_state)
    widgets["entry_namsx"].config(state=entry_state)
    widgets["entry_vin"].config(state=entry_state)
    widgets["date_dangkiem"].config(state=entry_state)
    widgets["date_baohiem"].config(state=entry_state)
    
    widgets["cbb_manv"].config(state=cbb_state) 
    widgets["cbb_trangthai"].config(state=cbb_state)
...
def clear_input(widgets):
    ...
    widgets["cbb_manv"].set("") 

    # ==================================
    # SỬA LỖI AttributeError TẠI ĐÂY
    # Dùng 'dt' (mô-đun) thay vì 'datetime' (lớp)
    default_date = dt.date.today() + dt.timedelta(days=365)
    # ==================================
...
def load_data(widgets):
    ...
    try:
        ...
        # Sửa: Chỉ khóa và set VIEW nếu có dữ liệu
        if tree.get_children():
             set_form_state(is_enabled=False, widgets=widgets)
             widgets["current_mode"] = "VIEW"
        # (Nếu không, clear_input() đã set mode="ADD")
...
def them_xe(widgets):
    ...
    driver_name_to_id = widgets["driver_name_to_id"]
    selected_name = widgets["cbb_manv_var"].get()
    manv = driver_name_to_id.get(selected_name, None)

    trangthai_value = 1
    if trangthai_text == "Bảo trì": trangthai_value = 0
    elif trangthai_text == "Hỏng": trangthai_value = 2
...
def on_item_select(event, widgets):
    ...
    try:
        ...
        driver_id_to_name = widgets["driver_id_to_name"]
        manv_id = data.MaNhanVienHienTai
        driver_name = driver_id_to_name.get(manv_id, "")
        widgets["cbb_manv"].set(driver_name)
        ...
        trang_thai_text = "Hoạt động"
        if data.TrangThai == 0: trang_thai_text = "Bảo trì"
        elif data.TrangThai == 2: trang_thai_text = "Hỏng"
...
def luu_xe_da_sua(widgets):
    ...
    try:
        ...
        cur.execute(
            sql,
            (
                loaixe, hangsx, dongxe, namsx_int, vin, ngay_dk,
                ngay_bh, trangthai_value, manv_sql,
                bienso, 
            ),
        )
        conn.commit()
...
# ================================================================
# HÀM QUAN TRỌNG NHẤT (SỬA LỖI LOGIC LƯU)
# ================================================================
def save_data(widgets):
    ...
    # Sửa: Dùng .get() để lấy 'current_mode' an toàn
...
def create_page(master):
    ...
    # ===== NÂNG CẤP =====
    ttk.Label(frame_info, text="Tên người lái:").grid(row=4, column=0, padx=5, pady=8, sticky="w")
    
    driver_id_to_name, driver_name_to_id, driver_display_list = _load_driver_data()
    
    cbb_manv_var = tk.StringVar()
    cbb_manv = ttk.Combobox(
        frame_info,
        textvariable=cbb_manv_var,
        values=driver_display_list, 
        width=28,
        state="readonly"
    )
    cbb_manv.grid(row=4, column=1, padx=5, pady=8, sticky="w")
    # ===== KẾT THÚC NÂNG CẤP =====
    ...
    cbb_trangthai = ttk.Combobox(
        frame_info, textvariable=cbb_trangthai_var, values=trangthai_options,
    ...
    tree = ttk.Treeview(
        frame_tree, columns=columns, show="headings", height=10,
    ...
    widgets = {
        ...
        "current_mode": "VIEW", # Sửa: Đặt giá trị mặc định
        
        # Thêm các widget và map mới
        "cbb_manv": cbb_manv,
        "cbb_manv_var": cbb_manv_var,
